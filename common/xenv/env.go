...
func ParseWithFuncs(v interface{}, funcMap map[reflect.Type]ParserFunc, opts ...Options) error {
	...
	return doParse(ref, parsers, opts, "ZEROIM")
}

func doParse(ref reflect.Value, funcMap map[reflect.Type]ParserFunc, opts []Options, parentFieldName string) error {
	refType := ref.Type()
	refTypeName := refType.Name()
	...
	for i := 0; i < refType.NumField(); i++ {
		...
		if refTypeField.Type.Kind() == reflect.Struct || (refTypeField.Type.Kind() == reflect.Ptr && refTypeField.Type.Elem().Kind() == reflect.Struct) {
			refTypeName = refTypeField.Name
		} else {
			refTypeName = ""
		}
		value, err := get(refTypeField, opts, parentFieldName+"_"+refTypeName)
		if err != nil {
			return err
		}
		if value == "" {
			if reflect.Struct == refField.Kind() {
				if err := doParse(refField, funcMap, optsWithPrefix(refType.Field(i), opts), parentFieldName+"_"+refTypeName); err != nil {
					return err
				}
			}
			continue
...
func get(field reflect.StructField, opts []Options, parentFieldName string) (val string, err error) {
	var exists bool
	var isDefault bool
	var loadFile bool
	var unset bool
	...
	if prefix == "" {
		prefix = strings.ToUpper(parentFieldName) + "_"
	}
